...
public class QwertyUtil
{
	...
	/**
	 * @description match search string with pinyinUnits,if success,save the
	 *              match keyword.
	 * @param pinyinUnits
	 *            pinyinUnits head node index
	 * @param pinyinUnitIndex
	 *            pinyinUint Index
	 * @param qwertyPinyinUnitIndex
	 *            pinyinBaseUnit Index
	 * @param baseData
	 *            base data for search.
	 * @param searchBuffer
	 *            search keyword.
	 * @param matchKeyword
	 *            save the match keyword.
	 * @return true if find,false otherwise.
	 */
	private static boolean findPinyinUnits(final List < PinyinUnit > pinyinUnits , int pinyinUnitIndex , int qwertyPinyinUnitIndex , final String baseData , StringBuffer searchBuffer , StringBuffer matchKeyword )
	{
		...
		if(pyUnit.isPinyin())
		{
			...
			{
				// The string of "search" is the string of
				// pinyinBaseUnit.getPinyin() of a subset. means match success.
				matchKeyword.append(baseData.charAt(pyUnit.getStartPosition()));
				searchBuffer.delete(0 ,searchBuffer.length());
				return true;
				...
				{ // match quanpin success
					// The string of pinyinBaseUnit.getPinyin() is the string of
					// "search" of a subset.
		...
		{ // non-pure Pinyin
			...
			{
				// The string of "search" is the string of
				// pinyinBaseUnit.getPinyin() of a subset.
				int startIndex = 0;
				...
				{ // match all non-pure pinyin
					...
					if((matchKeyword.length() <= 0))
					{
						...
						{
							// match case:[Non-Chinese characters]+[Chinese
							// characters]
							// for example:baseData="Tony测试"; match this
							// case:"onycs"
							// start [Non-Chinese characters]+[Chinese
							// characters]
							...
							// in fact,if pyUnit.isPinyin()==false,
							// pyUnit.getQwertyPinyinUnitIndex().size()==1. The
							// function of findPinyinUnits() will return false.
							boolean found = findPinyinUnits(pinyinUnits ,pinyinUnitIndex ,qwertyPinyinUnitIndex + 1 ,baseData ,searchBuffer ,matchKeyword);
							if(true == found)
							{
								return true;
