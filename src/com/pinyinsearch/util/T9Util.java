...
public class T9Util
{
	...
	/**
	 * T9 keyboard 1 2(ABC) 3(DEF) 4(GHI) 5(JKL) 6(MNO) 7(PQRS) 8(TUV) 9(WXYZ) *
	 * 0 #
	 */
	...
	private static boolean findPinyinUnits(final List < PinyinUnit > pinyinUnits , int pinyinUnitIndex , int t9PinyinUnitIndex , final String baseData , StringBuffer searchBuffer , StringBuffer matchKeyword )
	{
		...
		if(pyUnit.isPinyin())
		{
			...
			if(pinyinBaseUnit.getNumber().startsWith(search))
			{
				...
				// t9PinyinUnit.getNumber() of a subset. means match success.
				matchKeyword.append(baseData.charAt(pyUnit.getStartPosition()));
				searchBuffer.delete(0 ,searchBuffer.length());
				return true;
				...
				{ // match quanpin success
					// The string of t9PinyinUnit.getNumber() is the string of
					// "search" of a subset.
		...
		{ // non-pure Pinyin
			...
			{
				// The string of "search" is the string of
				// t9PinyinUnit.getNumber() of a subset.
				int startIndex = 0;
				...
				{ // match all non-pure pinyin
					...
					if((matchKeyword.length() <= 0))
					{
						...
						else
						{
							...
							// pyUnit.getPinyinBaseUnitIndex().size()==1. The
